# -*- coding: utf-8 -*-
"""СоциоГраф — аналитика программы «3Д Коммуникации». Светлая тёплая тема (Golos).
Граф D3 встроен (на тёмно-тёплом холсте). Панели — panels.py, грейды — grades.py.
...
# ── палитра светлой тёплой темы ──
INK = "#37312c"; MUTED = "#8c817a"; GRID = "rgba(55,49,44,0.08)"
...
def light(fig, title, height):
    fig.update_layout(template="plotly_white", height=height, margin=dict(l=10, r=10, t=44, b=10),
                      ...
                      font=dict(color=INK, family="Golos Text, sans-serif"), title=title)
...
<style>
 ...
 .stApp,[data-testid="stAppViewContainer"]{background:#faf9f5 !important;color:#37312c;}
 [data-testid="stHeader"]{background:rgba(250,249,245,.82) !important;backdrop-filter:blur(8px);border-bottom:1px solid #ece0d8;}
 .block-container{padding:3.4rem 2rem 2rem !important;}
 h1,h2,h3{font-family:'Golos Text',sans-serif !important;color:#37312c !important;font-weight:600 !important;letter-spacing:-.3px;}
 [data-testid="stMetricValue"]{font-family:'Golos Text',sans-serif;font-size:1.6rem !important;color:#e95f3e !important;}
 [data-testid="stMetricLabel"]{font-size:.72rem !important;color:#8c817a !important;text-transform:uppercase;letter-spacing:.8px;}
 [data-testid="stMetric"]{background:rgba(255,255,255,.72);backdrop-filter:blur(6px);border:1px solid #ece0d8;border-radius:14px;padding:.7rem 1rem;box-shadow:0 1px 3px rgba(55,49,44,.05);}
 [data-testid="stSidebar"]{background:#f5efe9 !important;border-right:1px solid #ece0d8;}
 [data-testid="stSidebar"] [data-testid="stWidgetLabel"],[data-testid="stSidebar"] label,[data-testid="stSidebar"] .stMarkdown,[data-testid="stSidebar"] h2,[data-testid="stSidebar"] [data-baseweb="radio"] div{color:#37312c !important;}
 .kpi-box{background:rgba(255,255,255,.72);backdrop-filter:blur(6px);border:1px solid #ece0d8;border-radius:16px;padding:1rem 1.1rem;min-height:96px;box-shadow:0 1px 3px rgba(55,49,44,.05);}
 .kpi-box .kpi-value{font-family:'Golos Text',sans-serif;font-size:1.55rem;font-weight:700;color:#e95f3e;line-height:1.1;}
 .kpi-box .kpi-label{font-size:.68rem;color:#8c817a;text-transform:uppercase;letter-spacing:.8px;margin-top:4px;}
 .kpi-box .kpi-sub{font-size:.72rem;color:#a89d95;margin-top:6px;}
 .section-header{font-family:'Golos Text',sans-serif;font-size:.78rem;text-transform:uppercase;letter-spacing:1.6px;color:#8c817a;border-bottom:1px solid #ece0d8;padding-bottom:6px;margin:1.6rem 0 1rem;font-weight:600;}
 .card{background:rgba(255,255,255,.72);backdrop-filter:blur(6px);border:1px solid #ece0d8;border-left:3px solid #e95f3e;padding:.9rem 1.1rem;border-radius:14px;margin:.5rem 0;font-size:.9rem;color:#37312c;line-height:1.6;box-shadow:0 1px 3px rgba(55,49,44,.04);}
 ...
 .muted{color:#8c817a;font-size:.82rem;}
 .info-box{background:rgba(255,255,255,.72);backdrop-filter:blur(6px);border:1px solid #ece0d8;border-radius:14px;padding:.9rem;margin:.6rem 0;font-size:.83rem;color:#8c817a;line-height:1.6;}
 .hero{font-family:'Golos Text',sans-serif;font-size:1.95rem;font-weight:700;color:#37312c;letter-spacing:-.5px;}
 .stTabs [data-baseweb="tab-list"]{gap:6px;border-bottom:none;}
 .stTabs [data-baseweb="tab"]{background:#fff;border:1px solid #ece0d8;border-radius:11px;padding:6px 16px !important;color:#8c817a;font-weight:500;}
 .stTabs [aria-selected="true"]{background:#e95f3e;color:#fff !important;border-color:#e95f3e;}
 .stTabs [data-baseweb="tab-highlight"],.stTabs [data-baseweb="tab-border"]{display:none !important;}
 .stButton>button,.stDownloadButton>button{background:#fff;color:#e95f3e;border:1px solid #ece0d8;border-radius:10px;}
 .stButton>button:hover,.stDownloadButton>button:hover{border-color:#e95f3e;}
 .sidebar-section{font-size:.7rem;text-transform:uppercase;letter-spacing:1.4px;color:#a89d95 !important;margin:1rem 0 .3rem;font-weight:600;}
</style>
...
def render_temporal(fd, emp):
    ...
    with c1:
        ...
        yrs = by_ym["y"].nunique()
        figs = go.Figure(go.Bar(x=[names[m] for m in typical.index], y=typical.round(0).values, marker_color=AMBER))
...
def main():
    ...
    st.markdown("""<div style="padding:.6rem 0 .2rem">
        <span style="font-family:'Golos Text',sans-serif;font-size:1.15rem;font-weight:700;color:#e95f3e;">🪡 3Д Коммуникации</span>
        <span style="color:#a89d95;font-size:.85rem;margin-left:10px;">Социальные технологии для бизнеса</span>
        </div>
        <div class="hero">Живая ткань признания</div>
        <div style="color:#8c817a;font-size:.9rem;margin-bottom:.4rem">ГК Термекс · программа «3Д Коммуникации»</div>""",
        unsafe_allow_html=True)
